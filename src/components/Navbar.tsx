...
function NavbarContent() {
  ...
  // Subtle cursor-tracking glow across the pill
  const handlePillMouseMove = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    glowRef.current.style.background = `radial-gradient(260px circle at ${x}px ${y}px, rgba(59,130,246,0.16), transparent 70%)`;
  };
  ...
  return (
    <>
      ...
      >
        ...
        <div className="absolute left-1/2 -top-5 h-[120px] w-[min(500px,100vw)] -translate-x-1/2 rounded-full bg-blue-500/10 blur-[90px] pointer-events-none animate-pulse" />
        <div className="absolute left-1/2 -top-2 h-[90px] w-[min(320px,80vw)] -translate-x-[60%] rounded-full bg-indigo-500/8 blur-[80px] pointer-events-none" />
        ...
        >
          ...
          <div
            ref={glowRef}
            className="absolute inset-0 z-0 rounded-full pointer-events-none mix-blend-screen transition-[background] duration-150"
          />
          ...
          <div className="relative z-10 flex items-center gap-4 sm:gap-6 min-w-0">
            <Link
              href="/"
              className="group flex min-w-0 shrink-0 items-center gap-2.5 text-sm font-bold tracking-tight text-white min-[375px]:text-base sm:text-lg"
            >
              <Image
                ...
                width={32}
                height={32}
                className="h-7 w-7 sm:h-8 sm:w-8 rounded-lg transition-transform duration-300 group-hover:scale-110 group-hover:drop-shadow-[0_0_10px_rgba(59,130,246,0.6)]"
                priority
              />
                  ...
                  >
                    {active && (
                      <motion.span
                        ...
                        className="absolute inset-0 -z-10 rounded-full bg-blue-500/10 border border-blue-400/10 shadow-[0_0_24px_rgba(59,130,246,.12)]"
          ...
          <div className="relative z-10 flex items-center gap-1 sm:gap-2 md:gap-3 shrink-0">
            ...
            {isSignedIn ? (
              ...
              <SignInButton mode="modal">
                ...
                >
                  <span className="pointer-events-none absolute inset-0 -translate-x-full [background:linear-gradient(110deg,transparent_40%,rgba(255,255,255,0.35)_50%,transparent_60%)] transition-transform duration-700 ease-out group-hover:translate-x-full" />
            ...
            <button
              type="button"
              className="lg:hidden grid min-h-11 min-w-11 place-items-center rounded-full text-white/50 hover:text-white hover:bg-white/[0.06] hover:shadow-[0_0_16px_rgba(59,130,246,0.15)] transition-all duration-200"
                  ...
                  >
                    <Link
                      ...
                      className={cn(
                        ...
                        isActive(link.href)
                          ? "text-blue-300 bg-blue-500/10 border border-blue-400/10 shadow-[0_0_18px_rgba(59,130,246,.10)]"
                ...
                <div className="mt-3 pt-3 border-t border-white/[0.07] flex flex-col gap-1.5">
                  ...
                  {!isSignedIn ? (
                    <SignInButton mode="modal">
                      <button
                        type="button"
                        className="group relative w-full mt-1 overflow-hidden px-5 py-3.5 rounded-xl text-base font-semibold text-white bg-gradient-to-r from-blue-600 to-blue-500 shadow-lg shadow-blue-900/30 hover:from-blue-500 hover:to-cyan-400 active:scale-[0.98] transition-all duration-200"
                      >
                        <span className="pointer-events-none absolute inset-0 -translate-x-full [background:linear-gradient(110deg,transparent_40%,rgba(255,255,255,0.35)_50%,transparent_60%)] transition-transform duration-700 ease-out group-hover:translate-x-full" />
                        <span className="relative z-10">Sign In</span>
                      </button>
